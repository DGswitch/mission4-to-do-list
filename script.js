...
const userDisplayMode = document.getElementById('user-display-mode'); 
...
const btnResetAllTasks = document.getElementById('btn-reset-all-tasks');
...
function getFormattedTime() {
    ...
    // Konversi jam ke format 12jam (1-12)
    hours = hours % 12;
...
function updateLiveTime() {
    ...
    const dayName = new Intl.DateTimeFormat('en-US', optionsWeekday).format(date);
    ...
    hours = hours ? hours : 12;
    hours = String(hours).padStart(2, '0');
    liveTimeSpan.textContent = `${dayName}, ${hours}:${minutes} ${ampm}`;
}
...
function renderTodoList() {
    ...
    .filter(task => {
        ...
        return task.date === currentFilterDate; 
    })
    ...
    filteredAndSortedTasks.forEach((task) => {
        ...
        ulContainer.classList.add('task-container');
        sectionContainer.appendChild(ulContainer);

        let liContainer = document.createElement('li');
        liContainer.classList.add('task-time');
        ulContainer.appendChild(liContainer);

        let divContainer = document.createElement('div');
        divContainer.classList.add('task-time-box');
...
document.addEventListener('DOMContentLoaded', () => {
    updateLiveTime();
    intervalId = setInterval(updateLiveTime, 1000); // 1detik?

    loadVerifiedId();
    loadTodoList();

    // saat submit user info
    if (userInfoForm) {
    ...
    // filter & input tanggal pada calender
    if (filterDateInput) {
    ...
    if (clearFilterBtn) {
        clearFilterBtn.addEventListener('click', () => {
            ...
            currentFilterDate = null;
            renderTodoList();
            showAlert("Date filter cleared.", "info");
        });
    }
});

// hapus interval saat page unloaded
